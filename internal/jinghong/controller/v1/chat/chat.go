...
func (cc *ChatController) GetMessagesBySession(c *gin.Context) {
	...
	uid, err := strconv.ParseUint(sessionId, 10, 64)
	if err != nil {
		core.WriteResponse(c, err, nil)
		return
	}

	messages, err := cc.b.ChatBiz().GetMessagesBySession(c, uint(uid), uint(user.ID))
	if err != nil {
		core.WriteResponse(c, err, nil)
		return
	}
...
func (cc *ChatController) DeleteSession(c *gin.Context) {
	...
	if err := cc.b.ChatBiz().DeleteSession(c, uint(uid), uint(user.ID)); err != nil {
		core.WriteResponse(c, err, nil)
		return
	}
